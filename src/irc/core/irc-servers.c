...
static void server_init_1(IRC_SERVER_REC *server)
{
	...
	server->isupport = g_hash_table_new((GHashFunc) g_istr_hash, (GCompareFunc) g_istr_equal);
	...
	irc_send_cmd_now(server, "JOIN 0");
	if (conn->starttls)
...
static void init_ssl_loop(IRC_SERVER_REC *server, GIOChannel *handle)
{
	...
	if (error & 1) { /* wait */
		server->starttls_tag =
		    g_input_add(handle, error == 1 ? G_INPUT_READ : G_INPUT_WRITE,
...
static void event_starttls(IRC_SERVER_REC *server, const char *data)
{
	...
	if (server->handle->readbuffer != NULL &&
		...
		int ret;
		char *str;
		ret = line_split("", -1, &str, &server->handle->readbuffer);
	}
